  ...
  <v-app id="inspire">
    ...
    <v-content>
      ...
      <div v-if="showDefault()">
        <v-toolbar dense flat dark color="purple darken-4" v-if="currentUser.kategori === 'Prodi'">
          <v-toolbar-items>
            <v-btn text to="/dashboard/list_users">Kirim surat</v-btn>
            <v-btn text to="/dashboard/list_pengumuman">Kirim pengumuman</v-btn>
          </v-toolbar-items>
          <!-- <v-toolbar-items v-else>
          <v-btn text to="/dashboard/list_users">
...
export default {
  ...
  methods: {
    showDefault() {
      if (
        this.$route.path === "/dashboard/list_users" ||
        this.$route.path === "/dashboard/list_pengumuman"
      ) {
        return true;
      } else {
        return false;
    ...
    checkKategori() {
      ...
      console.log(data[0]);
      if (hak_akses == "aktif") {
        if (data__kategori === "Admin") {
          this.menu__links = [
            { to: base__url + "/home", title: "Home", icon: "dashboard" },
            {
              to: base__url + "/new_posts",
              title: "Buat artikel",
              icon: "add"
            },
            {
        ...
        } else if (data__kategori === "Prodi") {
          // this.$router.replace(
          //   this.$route.query.redirect || "/dashboard/list_users"
          // );
          this.menu__links = [
            {
            ...
            // {
            //   to: base__url + "/surat",
            //   title: "Daftar surat yang terkirim",
            //   icon: "mail"
            // },
            {
              to: "/",
          ...
          // this.$router.replace(
          //   this.$route.query.redirect || "/dashboard/list_surat"
          // );
        ...
        this.menu__links = [
          ...
          {
            to: base__url + "/no_access",
            title: "Daftar surat",
            icon: "mail"
          },
          {
            to: base__url + "/no_accessp",
            title: "Pengumuman",
            icon: "announcement"
          }
        ];
